...
public class DenStream {
	...
	double lambda;
	double a;
	long t_p;//核心微簇退化为离群微簇的最段时间
	boolean isInitial = false;
	List<Point> initBuffer; //装读取到的数据的容器
	int initSize; //数据长度
	Set<MicroCluster> p_micro_cluster;
	Set<MicroCluster> o_micro_cluster;
	int mcid;
	List<Cluster> results; //潜在微簇的聚类结果
...
//				* (Math.log((minw) / (minw - 1))));
	...
	public void process(Point p) {  
		if (!isInitial) {
			...
			if (p.id % t_p == 0 && p.id != 0) {  //周期性检查微簇  并 聚类
	...
	public void generateCluster(long time) {
		...
		while (itr2.hasNext()) { //遍历潜在核心微簇
			...
			if (!mc.visited && mc.getWeight(time) > mu) {
				...
				results.add(C);
				Cid++;
				C.add(mc);  //为潜在核心微簇分配 簇id并且添加到簇中
				Queue<MicroCluster> queue = new LinkedList<MicroCluster>();
				queue.add(mc);
				while (!queue.isEmpty()) { //在潜在微簇列表中查找所有与这个微簇距离小于阈值的，满足条件的潜在核心微簇
					MicroCluster seed = queue.poll(); //删除队列中的第一个元素。如果为空，返回null
	...
	private void init() { // 如果大于 长度阈值，则开始计算
